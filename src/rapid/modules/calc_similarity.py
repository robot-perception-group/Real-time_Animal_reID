...
This module contains functions for similarity calculations using
k-nearest neighbors (k-NN). The functions leverage the Annoy library
 to efficiently find the nearest neighbors for descriptor vectors.

Functions:
- annoy_knn(database_index: AnnoyIndex, nr_knn: int, q_vec: np.ndarray)
 -> int, float: Finds the nearest neighbor(s) in the database for a given
  query vector using Annoy k-NN search and corresponding distance(s).
- similarity(query_animal: str, query_img_id: str, query_vecs: np.ndarray, map_dict:
dict, db_index: AnnoyIndex,
  nr_knn: int) -> pd.DataFrame: Computes the similarity of query vectors to the
  database and returns the results
  as a DataFrame with detailed match information.

Notes:
- The functions are optimized for speed using efficient k-NN search methods and
vectorized operations.
- The module assumes the use of an Annoy index and that descriptor vectors are in a
suitable format.
"""
...
def annoy_knn(database_index: AnnoyIndex, nr_knn: int, q_vec: np.ndarray) -> tuple:
    ...
    - match_distance (float): The distance between the query vector and the nearest
    neighbor.
...
def similarity(
    ...
    db_index: AnnoyIndex,
    nr_knn: int,
    query_animal: str = "unknown",
) -> pd.DataFrame:
    """
    Computes similarity for a set of query vectors to the database and returns the
    results as a DataFrame.
    This can be used for FalseTagFinding if the query animal ID is known, or if the
    animal ID is unknown and
    needs to be predicted, using "unknown" as the default for query_animal.

    Args:
    - query_animal (str): The animal ID for the query image, default is "unknown" if
    the ID is not provided.
    - query_img_id (str): The image ID for the query image.
    - query_vecs (np.ndarray): Query descriptor vectors.
    - map_dict (dict): A dictionary mapping indexes to animal and image IDs. It must
    contain 'animal_ids' and 'img_ids'.
    ...
    Raises:
    - ValueError: If the query vectors or mapping dictionary are not in the expected
    format.
    ...
    if (
        not isinstance(map_dict, dict)
        or "animal_ids" not in map_dict
        or "img_ids" not in map_dict
    ):
        raise ValueError(
            "Invalid map dictionary format. Must contain 'animal_ids' and 'img_ids'."
        )
    ...
    if not isinstance(nr_knn, int) or nr_knn <= 0:
        raise ValueError(
            "The number of nearest neighbors (nr_knn) must be a positive integer."
        )
...
# def get_sim_df(closed_similarity_data, glob_db_annoy_index, glob_idx_map,
# open_similarity_data, q_animal_id,
...
#         closed_match_idx, closed_match_distance = find_valid_knn(
#         glob_db_annoy_index, q_vec, same_img_idx)
#         closed_row = {
#             "q_vec_idx": q_vec_idx,
...
#             "n1_is_correct": (glob_idx_map['animal_ids'][closed_match_idx] ==
#             q_animal_id),
#         }
#         closed_similarity_data.append(closed_row)
#
#         # FALSE MATCH: remove all same animal indexes
#         open_match_idx, open_match_distance = find_valid_knn(glob_db_annoy_index,
#         q_vec, same_animal_idx)
#         open_row = {
#             "q_vec_idx": q_vec_idx,
#             "q_animal_id": q_animal_id,
#             "q_img_id": q_img_id,
...
#             "n1_is_correct": (glob_idx_map['animal_ids'][open_match_idx] ==
#             q_animal_id),
#         }
...
def similarity_w_idx_filtering(
    db_annoy_index,
    idx_map,
    q_animal_id,
    q_desc_vecs,
    ...
    for q_vec_idx, q_vec in enumerate(q_desc_vecs):
        if scenario == "closed":
            match_idx, match_distance = find_valid_knn_closed(
                db_annoy_index, q_vec, idx_to_filter, k_for_knn
            )
            if match_idx is None:
                continue
        elif scenario == "open":
            match_idx, match_distance = find_valid_knn_open(
                db_annoy_index, q_vec, idx_to_filter, k_for_knn
            )
...
def find_valid_knn_closed(annoy_index, query_vector, exclude_set, k_for_knn):
    neighbors = annoy_index.get_nns_by_vector(
        query_vector, k_for_knn, include_distances=True
    )
    for idx in range(0, len(neighbors[0])):
        if neighbors[0][idx] not in exclude_set:
            return neighbors[0][idx], neighbors[1][
                idx
            ]  # Return the first valid neighbor
    # print(f"q vec was {k_for_knn} times matched to the same img... No valid match
    # found.")
    return None, None
