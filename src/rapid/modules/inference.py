...
from rapid.modules.calc_similarity import similarity_w_idx_filtering
from rapid.modules.utils import get_ids_from_filename


def inference_w_confidence_scores(
    similarity_df: pd.DataFrame, n: int
) -> (str, pd.DataFrame):
    """
    Computes the most frequent animal ID using weighted similarity scores based on
    ...
    must include 'n1_animal_id' and 'n1_distance'.
    ...
    Raises:
    - ValueError: If `similarity_df` does not contain the required columns or if `n`
    is non-positive.
    """
    ...
    if not all(col in similarity_df.columns for col in ["n1_animal_id", "n1_distance"]):
        raise ValueError(
            "The similarity DataFrame must contain columns 'n1_animal_id' and "
            "'n1_distance'."
        )
    ...
    weighted_ids = (
        similarity_df.groupby("n1_animal_id")["reciprocal_distance"].sum().reset_index()
    )
    ...
    freq.columns = [
        "n1_animal_id",
        "freq",
    ]  # Rename columns to match the `weighted_ids` DataFrame
...
def get_weight_ratio(top_ids_df: pd.DataFrame) -> float:
    ...
    Args:
    - top_ids_df (pd.DataFrame): A DataFrame containing 'weighted_ids' column for top
    IDs.

    Returns:
    - float: The ratio between the first and second weighted ID. Returns 0 if only
...
def open_closed_behaviour(
    save_dir, db_imgs, topN_ids_match, sift_extractor, idx_map, db_annoy_index, n
):
    # PREPARE
    ...
    for curr_img_filename in tqdm(
        selected_imgs,
        desc="Analysing your database for confidence score calculations: ",
    ):
        #############################
        # Module-1: PreProc
        #############################
        ...
        # close
        same_img_idx = [
            i for i, val in enumerate(idx_map["img_ids"]) if val in q_img_id
        ]  # db position w current image
        ...
        # open
        same_animal_idx = [
            i for i, val in enumerate(idx_map["animal_ids"]) if val in q_animal_id
        ]  # db position w curr animal
        ...
        # Module-4: Inference
        #############################
        closed_glob_pred, closed_glob_topN_ids = inference_w_confidence_scores(
            closed_similarity_df, topN_ids_match
        )
        open_glob_pred, open_glob_topN_ids = inference_w_confidence_scores(
            open_similarity_df, topN_ids_match
        )
    ...
    kde_curve_ccm, kde_curve_cfm, kde_curve_om = KDE_curves(
        closed_set_correct_ratios, closed_set_false_ratios, open_set_ratios, save_dir
    )
...
def KDE_curves(
    closed_set_true_ratio, closed_set_false_ratio, open_set_ratios, save_dir
):
    # Handle None or empty inputs
    ratios = {
        "closed_correct": np.array(closed_set_true_ratio)
        if closed_set_true_ratio is not None
        else np.array([]),
        "closed_false": np.array(closed_set_false_ratio)
        if closed_set_false_ratio is not None
        else np.array([]),
        "open": np.array(open_set_ratios)
        if open_set_ratios is not None
        else np.array([]),
    }
    ...
    plt.savefig(
        os.path.join(save_dir, "confidence_score_hist_and_kde.svg"), format="svg"
    )
    plt.close()

    return (
        kde_results["closed_correct"],
        kde_results["closed_false"],
        kde_results["open"],
    )
