...
def false_tag_finder_plot(
    ...
    def gut_full_filename_from_img_id(imgs_dir: str, img_id: str):
        ...
        # Filter files where the third part of the split filename matches the search
        # string
        matching_files = [
            file
    ...
    def get_best_match_imgs(
        df: pd.DataFrame, id1: str, id2: str, id3: str, db_img: str
    ):
        result = []
        ...
        def get_top2_imgs(curr_id):
            ...
            # If multiple have the same frequency, calculate the average reciprocal
            # distance
            if len(frequent_values) > 1:
                avg_reciprocal_distance = frequent_values.groupby("n1_img_id")[
                    "reciprocal_distance"
                ].mean()
                most_frequent_img_id = avg_reciprocal_distance.idxmax()

            # Now get the 'n1_img_id' with the largest 'reciprocal_distance'
            largest_reciprocal_img_id = subset.loc[
                subset["reciprocal_distance"].idxmax(), "n1_img_id"
            ]
            ...
            if most_frequent_img_id == largest_reciprocal_img_id:
                result.append(
                    (
                        db_imgs + "/" + most_frequent_img_id,
                        db_imgs + "/" + largest_reciprocal_img_id,
                    )
                )
            else:
                result.append(
    ...
    def get_6_db_images_for_query(
        folder_path: str, animal_id: str, img_id: str, side: str
    ):
    ...
    merged_df = glob_topN_ids.merge(
        unique_image_count, left_on="n1_animal_id", right_index=True, how="left"
    )
    merged_df.rename(
        columns={
            "n1_animal_id": "Animal ID",
            "weighted_ids": "Weighted IDs",
            "freq": "Frequency",
            "n1_img_id": "Unique Image Count",
        },
        inplace=True,
    )
    ...
    q_db_img_paths = get_6_db_images_for_query(
        db_imgs, q_animal_id, q_img_id, side=filename.split("_")[1]
    )
    q_img_path = os.path.join(db_imgs, filename)
    top_match_img_ids = get_best_match_imgs(
        glob_sim_df,
        glob_topN_ids["n1_animal_id"].iloc[0],
        glob_topN_ids["n1_animal_id"].iloc[1],
        glob_topN_ids["n1_animal_id"].iloc[2],
        db_imgs,
    )
    ...
    if len(q_db_img_paths) == 1:
        image_paths = [
            ...
            # Row 1
            [
                top_match_img_ids[0][0],
                top_match_img_ids[0][1],
                q_db_img_paths[0],
                q_db_img_paths[0],
                q_db_img_paths[0],
            ],
    ...
    extra_string = [
        ...
        # Row 1
        [
            "Top1-MATCH (most match):",
            "Top1-MATCH (largest weight):",
            "Q in DB, example:",
            "Q in DB, example:",
            "Q in DB, example:",
        ],
    ...
    def prepare_img_titles(image_paths, extra_strings):
        if len(image_paths) != len(extra_strings):
            raise ValueError(
                "The length of image_paths and extra_strings must be the same"
            )
        ...
        for row_idx, row in enumerate(image_paths):
            processed_row = []
            extra_row = extra_strings[
                row_idx
            ]  # The extra row corresponding to this image row
            ...
            processed_paths.append(
                processed_row[:5]
            )  # Ensure no row exceeds 5 elements
    ...
    flattened_image_paths = [
        file for sublist in processed_image_paths for file in sublist
    ]
    ######################################
    #                PLOT
    ######################################
    ...
    scatter = big_ax.scatter(
        merged_df["Animal ID"],
        merged_df["Weighted IDs"],
        s=merged_df["Frequency"]
        * 25,  # Marker size based on occurrence frequency, scaled
    ...
    big_ax.set_title(
        f"ground truth: {q_animal_id}  VS.  predicted: {glob_pred} ({conf_score:.2f} %)"
    )
    ...
    for row in range(4):
        for col in range(5):
            ...
            if (
                not (row == 0 and col == 0)
                and not (row == 2 and col == 2)
                and not (row == 2 and col == 3)
                and not (row == 2 and col == 4)
                and not (row == 3 and col == 2)
                and not (row == 3 and col == 3)
                and not (row == 3 and col == 4)
            ):
                filename = flattened_image_paths[
                    row * 5 + col
                ]  # Index from the flattened list
                axes[row, col].text(
                    0.5,
                    1.05,
                    filename,
