...
from rapid.modules.img_processing import preproc, load_img
from rapid.modules.utils import get_ids_from_filename
from rapid.modules.calc_similarity import annoy_knn
...
def show_matched_keypoints(
    q_img, db_img, df_matches, preproc_width, orig_q_img=None, orig_db_img=None
):
    """
    Visualize matched keypoints with interactive slider and colormap, optionally over
    original images.
    ...
    if orig_q_img is not None and orig_db_img is not None:
        q_kps_orig = map_keypoints_to_original(
            df_matches["q_kp"].tolist(), orig_q_img.shape, preproc_width
        )
        db_kps_orig = map_keypoints_to_original(
            df_matches["n1_kp"].tolist(), orig_db_img.shape, preproc_width
        )
        q_img_color = (
            cv2.cvtColor(orig_q_img, cv2.COLOR_GRAY2BGR)
            if len(orig_q_img.shape) == 2
            else orig_q_img.copy()
        )
        db_img_color = (
            cv2.cvtColor(orig_db_img, cv2.COLOR_GRAY2BGR)
            if len(orig_db_img.shape) == 2
            else orig_db_img.copy()
        )
    else:
        q_kps_orig = df_matches["q_kp"].tolist()
        db_kps_orig = df_matches["n1_kp"].tolist()
        q_img_color = (
            cv2.cvtColor(q_img, cv2.COLOR_GRAY2BGR)
            if len(q_img.shape) == 2
            else q_img.copy()
        )
        db_img_color = (
            cv2.cvtColor(db_img, cv2.COLOR_GRAY2BGR)
            if len(db_img.shape) == 2
            else db_img.copy()
        )

    # ==== SORT MATCHES BY DISTANCE ====
    df_matches_sorted = df_matches.sort_values(
        "n1_distance", ascending=True
    ...
    norm_dist = (distances - distances.min()) / (
    ...
    def on_trackbar(val):
        ...
        for i in range(n_show):
            ...
            draw_keypoint_with_orientation(
                img_vis, pt1, q_kps_orig[i].size, q_kps_orig[i].angle, color, 1
            )
            draw_keypoint_with_orientation(
                img_vis, pt2, db_kps_orig[i].size, db_kps_orig[i].angle, color, 1
            )
        ...
        cv2.putText(
            img_vis,
            "Database Image",
            (w1 + int(10 * font_scale), int(30 * font_scale)),
            font,
            font_scale,
            color,
            font_thickness,
    ...
    cv2.createTrackbar(
        "Num Matches", window_name, 10, len(df_matches_sorted), on_trackbar
    )
    on_trackbar(10)
...
def verify_match_by_eye(
    query_img_path, predicted_id, database_path, nr_kps, preproc_width, on_original_imgs
):
    #############################################
    ...
    # ==== VISUALIZE ====
    show_matched_keypoints(
        q_img,
        db_img,
        filtered_df,
        preproc_width,
        orig_q_img=orig_q_img,
        orig_db_img=orig_n1_img,
    )


verify_match_by_eye(
    query_img_path="/data/demo/example_query/z42_left_img-0000787.jpg",
    predicted_id="z42",
