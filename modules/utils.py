...
def save_pickle(data_to_save: object, save_path: str, filename: str) -> None:
    """
    ...
    filename = f"{save_path}/{filename}.pkl"
    try:
...
def idx_maps_from_lists(animal_id_list: list, img_id_list: list) -> dict:
    """
    ...
    if len(animal_id_list) != len(img_id_list):
        raise ValueError("The length of animal_id_list and img_id_list must be the same.")

    img_id_array = np.array(img_id_list)
    animal_id_array = np.array(animal_id_list)
    idx_map = {'img_ids': img_id_array, 'animal_ids': animal_id_array}
    return idx_map
