...
def evaluate_acc(q_animal_id: str, top_ids_df: pd.DataFrame, R1_acc: int, R3_acc: int, R5_acc: int,
                 R10_acc: int, RR_k1: list, RR_k3: list, RR_k5: list, RR_k10: list) -> tuple:
    """
    Evaluates various ranking accuracy metrics based on the position of a query animal ID in the top IDs DataFrame.
    ...
    - top_ids_df (pd.DataFrame): A DataFrame containing the top ranked animal IDs and their associated ranks.
    ...
    Returns:
    - tuple: A tuple containing updated accuracy counts (R1_acc, R3_acc, R5_acc, R10_acc) and lists for Reciprocal Rank
      (RR_k1, RR_k3, RR_k5, RR_k10).

    Raises:
    - ValueError: If 'top_ids_df' is not a DataFrame or does not contain the expected 'n1_animal_id' column.
    ...
    if not all(isinstance(i, int) for i in [R1_acc, R3_acc, R5_acc, R10_acc]):
        raise TypeError("Rank accuracy counts (R1_acc, R3_acc, R5_acc, R10_acc) must be integers.")

    if not all(isinstance(i, list) for i in [RR_k1, RR_k3, RR_k5, RR_k10]):
        raise TypeError("Reciprocal Rank lists (RR_k1, RR_k3, RR_k5, RR_k10) must be lists.")

    # Get rank of the query animal
    if q_animal_id in top_ids_df['n1_animal_id'].head(10).values:
        rank = np.where(top_ids_df['n1_animal_id'].head(10).values == q_animal_id)[0][0] + 1
    else:
